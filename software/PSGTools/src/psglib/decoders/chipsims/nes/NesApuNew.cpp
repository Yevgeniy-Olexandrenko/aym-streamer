...
void NesApuNew::Init(int sampleRate, int cpuClock)
{
    m_cpuClock = cpuClock;
...
void NesApuNew::Write(uint16_t addr, uint8_t data)
{
    switch (addr)
    {
    case PULSE1DUTYVOL:
...
uint8_t NesApuNew::Read(uint16_t addr)
{
    if (addr == STATUS)
    {
        return (m_dmc.irq << 7
...
void NesApuNew::Process(uint32_t cpu_cycles)
{
...
void NesApuNew::CalculateSweepPulse1()
{
...
void NesApuNew::CalculateSweepPulse2()
{
...
void NesApuNew::UpdateQuarterFrame()
{
...
void NesApuNew::UpdateHalfFrame()
{
...
uint8_t NesApuNew::CpuRead(uint16_t addr)
{
    // TODO
