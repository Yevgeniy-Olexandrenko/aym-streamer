...
const Frame& SwapChannelsOrder::operator()(const Chip& chip, const Frame& frame)
{
    ...
    if (chip.output() == Chip::Output::Stereo && chip.stereo() != Chip::Stereo::ABC)
    {
        ...
        {
            Frame::Channel lchan = m_frame[chip].ReadChannel(l);
            Frame::Channel rchan = m_frame[chip].ReadChannel(r);
            m_frame[chip].UpdateChannel(l, rchan);
            m_frame[chip].UpdateChannel(r, lchan);
        };
