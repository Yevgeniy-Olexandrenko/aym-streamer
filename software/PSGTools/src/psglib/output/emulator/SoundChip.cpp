...
static const uint32_t DUTY_CYCLES[9] =
{
	0x80000000, // 3.125 %
	0xc0000000, // 6.25 %
	0xf0000000, // 12.50 %
...
static const uint8_t mapping8914to8910[] = { 0, 2, 4, 11, 1, 3, 5, 12, 7, 6, 13, 8, 9, 10, 14, 15 };
...
SoundChip::SoundChip(ChipType chipType, PSGType psgType, uint32_t clockRate, uint32_t sampleRate)
	: m_chipType(chipType)
	, m_mode(0)
...
void SoundChip::Reset()
{
	...
	for (int i = 0; i < AY_PORTA; i++) SoundChip::Write(i, 0);
}
...
void SoundChip::Compute(double& outL, double& outR)
{
	...
	for (int chan = 0; chan < NUM_CHANNELS; chan++)
	{
		m_tone[chan].Update(is_expanded_mode());
	}

	m_noise.Update(is_expanded_mode(), m_chipType == ChipType::AY8930);

	for (int chan = 0; chan < NUM_CHANNELS; chan++)
	{
#if 0
		m_vol_enabled[chan] = (m_tone[chan].GetOutput() & ~tone_enable(chan)) | (m_noise.GetOutput() & ~noise_enable(chan));
#else
		m_vol_enabled[chan] = (m_tone[chan].GetOutput() | tone_enable(chan)) & (m_noise.GetOutput() | noise_enable(chan));
#endif
	}
	...
	{
		int output = 0;
		if (m_vol_enabled[chan])
		{
			int tone_envelope = m_tone[chan].GetEField(is_expanded_mode(), m_chipType == ChipType::AY8914);

			if (tone_envelope != 0)
			{
				EnvelopeUnit& envelope = m_envelope[is_expanded_mode() ? chan : 0];
			...
			{
				output = m_tone[chan].GetVolume(is_expanded_mode());
			}
		}
