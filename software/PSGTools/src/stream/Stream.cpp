...
void Stream::Loop::ComputeExtraLoops()
{
	...
	if (available())
	{
		...
		auto maxPlaybackFrames = (k_prefDurationMM * 60 + k_prefDurationSS) * m_stream.playback.frameRate();
...
size_t Stream::Playback::framesCount() const
{
...
const Frame& Stream::Playback::getFrame(FrameId frameId) const
{
	auto frameCount = m_stream.framesCount();
...
void Stream::Playback::ComputeDuration(size_t frameCount, int& hh, int& mm, int& ss, int& ms) const
{
...
void Stream::Playback::ComputeDuration(size_t frameCount, int& hh, int& mm, int& ss) const
{
